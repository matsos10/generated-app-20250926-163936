...
import type { Message, ToolCall, TicketPriority, TicketSentiment } from './types';
...
export class ChatHandler {
  ...
  /**
   * Analyze ticket content to determine priority and sentiment
   */
  async analyzeTicketContent(title: string, description: string): Promise<{ priority: TicketPriority; sentiment: TicketSentiment }> {
    const systemPrompt = `You are an AI assistant for a customer support platform. Analyze the following ticket content and return a JSON object with two keys: "priority" (one of "Low", "Medium", "High") and "sentiment" (one of "Positive", "Neutral", "Negative"). Base the priority on urgency and impact. Base sentiment on the user's tone. Respond ONLY with the JSON object.`;
    ...
    try {
      ...
      const validSentiments: TicketSentiment[] = ['Positive', 'Neutral', 'Negative'];
      if (!validPriorities.includes(parsed.priority) || !validSentiments.includes(parsed.sentiment)) {
        throw new Error('Invalid priority or sentiment value in AI response.');
      }
      return {
      ...
      // Fallback to default values in case of AI failure
      return {
        priority: 'Medium',
        sentiment: 'Neutral',
      };
  ...
  private async handleStreamResponse(
    ...
    if (accumulatedToolCalls.length > 0) {
      ...
      // Stream the final response as well
      onChunk(finalResponse);
      return { content: fullContent + finalResponse, toolCalls: executedTools };
    }
